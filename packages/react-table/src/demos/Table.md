...
class BulkSelectTableDemo extends React.Component {
  constructor(props) {
    ...
    this.handleSelectClick = newState => {
      if (newState === 'none') {
        ...
        let newRows = [];
        let test = this.state.rows.map(post => {
        ...
        let newRows = [];
        for (var i = 1; i <= 75; i++) newRows = [...newRows, i];
  ...
  buildSelectDropdown() {
    const { isDropDownOpen, selectedItems, res } = this.state;
    const numSelected = selectedItems.length;
    const allSelected = numSelected === res.length;
        ...
        <PageSection isWidthLimited>
          ...
          {!loading && (
            <Table
              ...
              rows={(this.state.page === 1 ? res : rows)
                .map(post => ({
                  cells: [
                    post.servers,
                    post.threads,
